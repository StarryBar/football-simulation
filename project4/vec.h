...
//
	struct tag3 {
		...
		GLuint  sp;
		GLuint  pt;
		tag3(GLuint s = GLuint(0)) :
			dr(s), sp(s), pt(s) {}

		tag3(GLuint x, GLuint y, GLuint z) :
			dr(x), sp(y), pt(z) {}

		GLuint& operator [] (int i) { return *(&dr + i); }
		const GLuint operator [] (int i) const { return *(&dr + i); }

		friend std::ostream& operator << (std::ostream& os, const tag3& v) {
			return os << "( " << v.dr << ", " << v.sp << ", " << v.pt << " )";
		}

		friend std::istream& operator >> (std::istream& is, tag3& v)
		{
			return is >> v.dr >> v.sp >> v.pt;
		}

		operator const GLuint* () const
		{
			return static_cast<const GLuint*>(&dr);
		}
		operator GLuint* ()
		{
			return static_cast<GLuint*>(&dr);
		}
...
struct vec4 {
    ...
    vec4 operator * ( const vec4& v ) const
	{ return vec4( x*v.x, y*v.y, z*v.z, w*v.w ); }//wrong
